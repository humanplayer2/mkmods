# Mechanical Keyboard Mods

Notes and photos of mechanical keyboards mods and builds.

## Handwired Builds
- [Laser Prosthetic: Trackpoint ergo laser-cut from acrylic and MDF](LaserProsthetic/laser.md)
- [Frejas Hitbox: A gamepad with a case made of HAMA beads](FrejasHitbox/frejas.md)
- [Lite-Puter Footpanel: Light-controller rewired to run QMK](Lite-Puter/lite-puter.md)

## Mechanical Mods
- [Anne Pro 2: Hotswap mod using cheap rivets](AnnePro2/RivetHotswap.md)

## Builds
- [Tokyo60: Some notes on QMK/VIA](tokyo60/tokyo60.md)
